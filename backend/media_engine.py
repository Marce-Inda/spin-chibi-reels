...
class MediaEngine:
    ...
    @staticmethod
    async def generate_narration_audio(scene: Scene, output_audio_path: str):
        """Generates voiceover audio using edge-tts (free, expressive neural Spanish voice)."""
    ...
    def create_fallback_chibi_frame(scene: Scene, frame_path: str, width: int = 720, height: int = 1280):
        ...
        for i in range(3):
            ...
            symbol = "🎰" if scene.id < 4 else "7️⃣"
            try:
                draw.text((rx + 25, 420), symbol, fill=(255, 255, 255))
            except Exception:
                pass
        
        # Draw 3D Chibi Avatar Head with Huge Sparkling Star Eyes & Modern Hair/Outfit
        head_cx, head_cy = width // 2, 920
        head_r = 130
        
        # Modern Trendy Outfit (Luxury Blazer / Jacket)
    ...
    @staticmethod
    def generate_synthetic_audio_effect(effect_type: str, output_sfx_path: str):
        """Generates a clean synthetic sound effect using FFmpeg filters."""
    ...
    async def render_scene_video(cls, scene: Scene, scene_dir: str) -> str:
        """Renders video clip for a single scene (combining image frame + TTS narration + sound effect)."""
        ...
        # 2. Create TTS Audio Narration
        await cls.generate_narration_audio(scene, narration_path)

        # 3. Create SFX Audio
        cls.generate_synthetic_audio_effect(scene.sound_effect, sfx_path)

        # 4. Render 9:16 Video Clip with subtle motion zoom & combined audio using FFmpeg
        ...
        ffmpeg_cmd = (
            ...
            f"-map \"[v]\" -map \"[a]\" -c:v libx264 -preset fast -pix_fmt yuv420p -t {duration} {clip_video_path}"
        )
    ...
    async def assemble_reel(cls, scenes: List[Scene], output_filename: str = "casino_reel.mp4") -> str:
        """Stitches all scene video clips into the final vertical 9:16 Reel."""
        ...
        # Concat video clips cleanly
        concat_cmd = (
            f"ffmpeg -y -f concat -safe 0 -i {list_txt_path} "
            f"-c:v libx264 -preset fast -c:a aac -b:a 192k {final_output_path}"
        )
        subprocess.run(concat_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
